...
class TestGetCleaning:
    ...
    async def test_wrong_id_returns_error(self, app: FastAPI, client: TestClient, id: int, status_code: int) -> None:
...
class TestUpdateCleaning:
    ...
    async def test_update_cleaning_with_valid_input(
        ...
        for i in range(len(attrs_to_change)):
            attrs_to_change = getattr(updated_cleaning, attrs_to_change[i])
            assert attrs_to_change != getattr(test_cleaning, attrs_to_change[i])
            assert attrs_to_change == values[i]
